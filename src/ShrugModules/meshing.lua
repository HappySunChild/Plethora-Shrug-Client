	...
	for _, node in ipairs(map:GetNodes()) do
		if not node.Visited then
			...
			local displayData = {
				...
				Label = ('%s\n(%d)'):format(node.Name, count),
				
				MetaState = node.Data.MetaState,
				WhitelistData = node.Data.WhitelistData
			}
...
local meshing = {}
meshing.MeshTypes = { --- @type table<string, MeshingFunction>
	bounding = function (objects)
		local map = meshing.newMap()
	...
	legacy = function (objects)
		...
		for _, object in ipairs(objects) do
			local displayData = { --- @type DisplayData
				X = object.X,
				Y = object.Y,
				Z = object.Z,
				...
				Color = object.WhitelistData.Color,
				WhitelistData = object.WhitelistData
			}
			
			table.insert(display, displayData)
		end
...
---@return DisplayData[]
function meshing.doMeshing(meshType, objects)
	local callback = meshing.MeshTypes[meshType or 'bounding']
